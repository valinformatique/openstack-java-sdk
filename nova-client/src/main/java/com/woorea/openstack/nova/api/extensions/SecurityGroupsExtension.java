...
public class SecurityGroupsExtension {
    ...
    public ListSecurityGroupsByServer listSecurityGroupsByServer(String serverId) {
        return new ListSecurityGroupsByServer(serverId);
    }

    public class ListSecurityGroupsByServer extends OpenStackRequest<SecurityGroups> {

        public ListSecurityGroupsByServer(String serverId) {
            super(CLIENT, HttpMethod.GET, new StringBuilder("/servers/").append(serverId).append("/os-security-groups"), null, SecurityGroups.class);
        }
    }
}
